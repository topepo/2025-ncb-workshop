...
author: "Max Kuhn, E. David Aja, Shannon Pileggi"
...
On windows it is the user's homew directory (aka `%USERPROFILE%` environment variable)
...
> Adds `.Rproj.user`, `.Rhistory`, `.Rdata`, `.httr-oauth`, .`DS_Stor`e, and `.quarto` to your global (a.k.a. user-level) `.gitignore`. This is good practice as it decreases the chance that you will accidentally leak credentials to GitHub.
...
## A little bit on partial automation
 ...
 If your project requires many computing iterations because it:
 
 - requires a lot of data transformations _and_ 
...
You'll want to make sure that the raw data and intermediary data are consistent with the results that you are giving to your clients. 
